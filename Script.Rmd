...
```{r}
###1.1.2 Split into Testing and Training Data
#haw_dat <- read.csv("Data/Processed/Haw_5x5_envadded.csv")
...
out<- gjam(formula = formula, haw_train_x, haw_train_y, modelList = ml)
save(out, file = "Models/trained_hawaii_origformula.rdata")
load("Models/trained_hawaii_origformula.rdata")
```
...
for(i in 1:length(steps)){
  name <- steps[i]
  namey <- adex[i]
  yo = newy2[,cdex[i]] #if we want them all to match I think it needs to be in catch/1000 hooks
yp = p1$sdList$yMu[,namey]
yp <- as.data.frame(yp)
yp$cluster <- clusters
