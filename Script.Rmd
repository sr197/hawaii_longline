...
2. Selects the best set of variables for predicting hawaii CPUE based on the environment. 
  - This is currently in progress, we will most likely be selecting variables based on BOTH hawaii and 
    overlapping IATTC 
...
IATTC_dat_0 <- IATTC_dat %>% filter(LatC5 > 0)


IATTC_dat_0_x <-  IATTC_dat_0[,xnames]
IATTC_dat_0_y <-  IATTC_dat_0[,cdex]

haw_0_x <-  haw_x[,xnames]
haw_0_y <-  haw_y[,cdex]

o_y <- rbind(IATTC_dat_0_y, haw_0_y)
o_x <- rbind(IATTC_dat_0_x, haw_0_x)
...
out<- gjam(formula = formula, o_x, o_y, modelList = ml)
save(out, file = "Models/above0_full.rdata")

plotPars <- list(PLOTALLY = T, SMALLPLOTS = T, GRIDPLOTS = T, 
                 SAVEPLOTS = T)
gjamPlot(out, plotPars = plotPars) #jim said to look at inverse prediction 
load("Models/above0_full.rdata")
```


##Dredge selection loop
Use the overlapping hawaii and IATTC teleost species dataset created above (overlap_y and overlap_x. 
Use 70% of the data. 
Run the models on all combinations of variables. 
Predict on the remaining 30% 
Calculate RMSE and Rsquared for each species and the total dataset. 

```{r}
#test to remember how dredgeGJAM works 
covars<- c('A','B','C','A:B')
allformulas<- dredgeGJAM(covars)
allformulas<- dredgeGJAM(covars, alwaysIn='E+F+G')
...
xnames <- c("DEPTH.MEAN","MAX.DEPTH","TEMP.EPI",
            "OXY.MESO","salinity.0000m.mean","surf.el.mean","water.temp.0000m.mean","aqua.CHL.chlor.a.mean") #I'm taking out the different version of similar things (i.e min depth and depth range were not coming out as important in inverse pred, so just use depth.mean. just use temp.epi and oxy meso)
cdex <- c("BUMn", "MLSn", "SFAn", "SSPn", "SWOn", "ALBn", "BETn", "SKJn", "YFTn")
...
overlap_all <- cbind(overlap_x, overlap_y)

smp_size <- floor(0.70 * nrow(overlap_all))
set.seed(123)
train_ind <- sample(seq_len(nrow(overlap_all)), size = smp_size)
train <- overlap_all[train_ind, ]
test <- overlap_all[-train_ind, ]
...
S <- ncol(ydata)
typeNames <- rep("DA", S)
ml <- list(ng = 100, burnin = 40, typeNames = typeNames)
ml$REDUCT <- TRUE
#Covars are the parameters which are not in the base and which we want to iterate
#Always-in are the base parameters we know we want to include. Salinity chla and depth meancame out as important in inverse pred x. 
#This reduces the number of iterations in the for loop

#find all possible interactions and variables
inter <- model.matrix( ~.^2, data=xdata) 
covars <- colnames(inter)
covars <- covars[-1]
covars <- covars[-8] #I;ll put chla in always in because I know its important 

allformulas<- dredgeGJAM(covars, alwaysIn='aqua.CHL.chlor.a.mean')
savestuff<- as.data.frame(matrix(NA, length(allformulas), 17))

colnames(savestuff) = c("Formula", "DIC", "Model", "RMSE","BUMn", "MLSn", "SFAn", "SSPn", "SWOn", "ALBn", "BETn", "SKJn", "YFTn", "Non_R2_By", "R2_By","Non_R2_Total", "R2_Total")
pb <- txtProgressBar(0, length(allformulas), style = 3)
for(M in 1:length(allformulas)){
    form <- as.formula(allformulas[[M]])
    model    <- try(gjam(form, xdata, ydata, modelList = ml), T)
    name <- paste('Model', M, sep="")
    file <- paste('Model', M, '.Rdata', sep="")
    ...
    non_conditional_lm <- lm(rowSums(out_of_sample_prediction_non_conditional$sdList$yMu) ~ rowSums(ycFinal))
    proj_ByCatch_Con <- out_of_sample_prediction_conditional$sdList$yMu[,MarketRFMO]
    proj_ByCatch_non_Con <- out_of_sample_prediction_non_conditional$sdList$yMu[,MarketRFMO]
    proj_con_lm <- lm(rowSums(proj_ByCatch_Con) ~ rowSums(ycByCatch))
    proj_non_con_lm <- lm(rowSums(proj_ByCatch_non_Con) ~ rowSums(ycByCatch))
    ###
    if(inherits(model, 'try-error'))next
    savestuff[M,1]<- allformulas[[M]]
    savestuff[M,2]<- model$fit$DIC
    savestuff[M,3]<- name
    savestuff[M,4]<- model$fit$rmspeAll
    savestuff[M,5]<- model$fit$rmspeBySpec[1]
    savestuff[M,6]<- model$fit$rmspeBySpec[2]
    savestuff[M,7]<- model$fit$rmspeBySpec[3]
    savestuff[M,8]<- model$fit$rmspeBySpec[4]
    savestuff[M,9]<- model$fit$rmspeBySpec[5]
    savestuff[M,10]<- model$fit$rmspeBySpec[6]
    savestuff[M,11]<- model$fit$rmspeBySpec[7]
    savestuff[M,12]<- model$fit$rmspeBySpec[8]
    savestuff[M,13]<- model$fit$rmspeBySpec[9]
    savestuff[M,14]<- model$fit$rmspeBySpec[10]
    savestuff[M,15]<- model$fit$rmspeBySpec[11]
    savestuff[M,16]<- model$fit$rmspeBySpec[12]
    savestuff[M,17]<- model$fit$rmspeBySpec[13]
    savestuff[M,18]<- model$fit$rmspeBySpec[14]
    savestuff[M,19]<- model$fit$rmspeBySpec[15]
    savestuff[M,20]<- model$fit$rmspeBySpec[16]
    savestuff[M,21]<- model$fit$rmspeBySpec[17]
    savestuff[M,22]<- model$fit$rmspeBySpec[18]
    savestuff[M,23]<- model$fit$rmspeBySpec[19]
    savestuff[M,24]<- model$fit$rmspeBySpec[20]
    savestuff[M,25]<- model$fit$rmspeBySpec[21]
    savestuff[M,26]<- model$fit$rmspeBySpec[22]
    savestuff[M,27]<- model$fit$rmspeBySpec[23]
    savestuff[M,28]<- format(summary(proj_non_con_lm)$adj.r.squared, digits=4)
    savestuff[M,29]<- format(summary(proj_con_lm)$adj.r.squared, digits=4)
    savestuff[M,30]<- format(summary(conditional_lm)$adj.r.squared, digits=4)
    savestuff[M,31]<- format(summary(non_conditional_lm)$adj.r.squared, digits=4)
...
avr_y_actual <- mean(as.numeric(as.matrix(observed[specnames[i]])))

# 2.2. Total sum of squares
ss_total <- sum((observed[specnames[i]] - avr_y_actual)^2)

# 2.3. Regression sum of squares
ss_regression <- sum((predicted[specnames[i]] - avr_y_actual)^2)

# 2.4. Residual sum of squares
ss_residuals <- sum((predicted[specnames[i]] - observed[specnames[i]])^2)
...
formula <- as.formula(~aqua.CHL.chlor.a.mean + 
                        salinity.0000m.mean + MAX.DEPTH)
...
save(out, file = "Models/trained_hawaii_tempsalchlamaxdepth.rdata")
load("Models/trained_hawaii_tempsalchlamaxdepth.rdata")
```
...
IATTC_dat_US <- IATTC_dat %>% filter(Flag == "USA")


IATTC_dat_US_x <-  IATTC_dat_US[,xnames]
IATTC_dat_US_y <-  IATTC_dat_US[,cdex]



xnames <- c(rownames(out[["parameters"]][["sensTable"]]))
#aggregate
cvar <- IATTC_dat_US_x[,xnames]
minSize <- 10                   # experiment with sizes of clusters
maxSize <- 20
...
IATTC_dat_US_y$cluster <- clusters
newdata <- left_join(IATTC_dat_US_y, xmean, by = "cluster")
newy2 <- as.data.frame(newdata) %>% group_by(cluster) %>% summarise_at(cdex, mean, na.rm =T)
newy2 <- newy2[,-1]
...
yp = p3$sdList$yMu[,namey]
yp <- as.data.frame(yp)
yp$Flag <- IATTC_dat$Flag
yp <- yp %>% filter(Flag == "USA")
yp$cluster <- clusters
yp <- yp %>% group_by(cluster) %>% summarise(yp = mean(yp, na.rm =T))
yp <- yp[,-1]
dat <- cbind(yo, yp)
...
do.call(ggarrange, c(plot_list[1:length(plot_list)], common.legend = TRUE, legend = "bottom"))+ ggsave(filename= "Figures/FigureS_unconditional_outofsample_IATTC_US_clustered.jpg", plot=last_plot(), width = 30, height=25, units=c("cm"), dpi=500)
```
###Same extent
Same as above but just the same lat and long)
...
for(i in 1:length(steps)){
  name <- steps[i]
  namey <- cdex[i]
  yo = IATTC_dat_overlap_y[,namey]
yp = p3$sdList$yMu[,namey]
yp <- as.data.frame(yp)
yp$Lat <- IATTC_dat$LatC5
yp$Lon <- IATTC_dat$LonC5
yp <- yp %>% filter(Lat > min(coords$LAT) & Lat < max(coords$LAT)& Lon > min(coords$LON) & Lon < max(coords$LON))
...
do.call(ggarrange, c(plot_list[1:length(plot_list)], common.legend = TRUE, legend = "bottom"))+ ggsave(filename= "Figures/FigureS_unconditional_outofsample_IATTC_overlap.jpg", plot=last_plot(), width = 30, height=25, units=c("cm"), dpi=500)
```
###Same extent clustered
Same as above but just the same lat and long)
...
xnames <- c(rownames(out[["parameters"]][["sensTable"]]))
#aggregate
cvar <- IATTC_dat_overlap_x[,xnames]
minSize <- 10                   # experiment with sizes of clusters
maxSize <- 20
...
IATTC_dat_overlap_y$cluster <- clusters
newdata <- left_join(IATTC_dat_overlap_y, xmean, by = "cluster")
newy2 <- as.data.frame(newdata) %>% group_by(cluster) %>% summarise_at(cdex, mean, na.rm =T)
newy2 <- newy2[,-1]
...
do.call(ggarrange, c(plot_list[1:length(plot_list)], common.legend = TRUE, legend = "bottom"))+ ggsave(filename= "Figures/FigureS_unconditional_outofsample_IATTC_overlap_clustered.jpg", plot=last_plot(), width = 30, height=25, units=c("cm"), dpi=500)
```
...
do.call(ggarrange, c(plot_list[1:length(plot_list)], common.legend = TRUE, legend = "bottom"))+ ggsave(filename= "Figures/FigureS_unconditional_outofsample_IATTC_above0.jpg", plot=last_plot(), width = 30, height=25, units=c("cm"), dpi=500)
```
###Above0 clustered
Same as above but just the same lat and long)
...
IATTC_dat_0_y$cluster <- clusters
newdata <- left_join(IATTC_dat_0_y, xmean, by = "cluster")
newy2 <- as.data.frame(newdata) %>% group_by(cluster) %>% summarise_at(cdex, mean, na.rm =T)
newy2 <- newy2[,-1]
...
do.call(ggarrange, c(plot_list[1:length(plot_list)], common.legend = TRUE, legend = "bottom"))+ ggsave(filename= "Figures/FigureS_unconditional_outofsample_IATTC_above0_clustered.jpg", plot=last_plot(), width = 30, height=25, units=c("cm"), dpi=500)
```
...
load("Models/trained_hawaii_salchlamaxdepth.rdata") #this is on original CPUE (G) and a simplified formula. These are the variables that came out as important when I ran a model on hawaii and overlapping IATTC teleosts.  
# aqua.CHL.chlor.a.mean + salinity.0000m.mean + MAX.DEPTH


load("Models/overlap_full.rdata") #this is trained on hawaii and overlapping IATTC teleosts with all of the env. variables. Jim suggested looking at the inverse prediction for x to see which variables are important for prediction. It looks like chla, salinity, and maximum depth are most closesly back-predicted to the 1:1 line. 

load("Models/trained_hawaii_tempsalchlamaxdepth.rdata") #this is on original CPUE (G) and a simplified formula.
# water.temp.0000m.mean + aqua.CHL.chlor.a.mean + salinity.0000m.mean + MAX.DEPTH
