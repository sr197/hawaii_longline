...
IATTC_with_env_allyears <- read.csv("Data/Raw/IATTC_with_env_allyears.csv") # use this later for lat long
...
covars <- covars[-1]
covars <- covars[-6] #I;ll put chla, salinity and depth.mean in always in because I know its important 
covars <- covars[-3]
covars <- covars[-1]
allformulas<- dredgeGJAM(covars, alwaysIn='aqua.CHL.chlor.a.mean + salinity.0000m.mean + DEPTH.MEAN')
write.csv(allformulas, "Models/allformulas.csv")
savestuff<- as.data.frame(matrix(NA, length(allformulas), 23))

colnames(savestuff) = c("Formula", "DIC", "R2", "RMSE", "Mean_obs", "R2BUMn", "R2MLSn", "R2SFAn", "R2SSPn", "R2SWOn", "R2ALBn", "R2BETn", "R2SKJn", "R2YFTn", "RMSEBUMn", "RMSEMLSn", "RMSESFAn", "RMSESSPn", "RMSESWOn", "RMSEALBn", "RMSEBETn", "RMSESKJn", "RMSEYFTn")
...
for(M in 1:length(allformulas)){
    ...
    y_obs <- ytest 
    y_pred_long <- as.data.frame(y_pred) %>% pivot_longer(cols = BUMn:YFTn, names_to = c("spec"))
    y_obs_long <- as.data.frame(y_obs) %>% pivot_longer(cols = BUMn:YFTn, names_to = c("spec"))
    
    total <- cbind(y_pred_long$value, y_obs_long$value)
    colnames(total) <- c("pred", "obs")
    ...
    savestuff[M,3] <- r2_general(total$pred, total$obs)
    savestuff[M,4] <- RMSE_func(actual = total$obs, pred = total$pred) 
    savestuff[M,5] <- mean(total$obs)
    savestuff[M,6] <- r2_general(y_pred[,1], y_obs[,1])
    savestuff[M,7] <- r2_general(y_pred[,2], y_obs[,2])
    savestuff[M,8] <- r2_general(y_pred[,3], y_obs[,3])
    savestuff[M,9] <- r2_general(y_pred[,4], y_obs[,4])
    savestuff[M,10] <- r2_general(y_pred[,5], y_obs[,5])
    savestuff[M,11] <- r2_general(y_pred[,6], y_obs[,6])
    savestuff[M,12] <- r2_general(y_pred[,7], y_obs[,7])
    savestuff[M,13] <- r2_general(y_pred[,8], y_obs[,8])
    savestuff[M,14] <- r2_general(y_pred[,9], y_obs[,9])
    savestuff[M,15] <- RMSE_func(actual = y_obs[,1], pred = y_pred[,1]) 
    savestuff[M,16] <- RMSE_func(actual = y_obs[,2], pred = y_pred[,2])
    savestuff[M,17] <- RMSE_func(actual = y_obs[,3], pred = y_pred[,3])
    savestuff[M,18] <- RMSE_func(actual = y_obs[,4], pred = y_pred[,4])
    savestuff[M,19] <- RMSE_func(actual = y_obs[,5], pred = y_pred[,5])
    savestuff[M,20] <- RMSE_func(actual = y_obs[,6], pred = y_pred[,6])
    savestuff[M,21] <- RMSE_func(actual = y_obs[,7], pred = y_pred[,7])
    savestuff[M,22] <- RMSE_func(actual = y_obs[,8], pred = y_pred[,8])
    savestuff[M,23] <- RMSE_func(actual = y_obs[,9], pred = y_pred[,9])
    setTxtProgressBar(pb, M)
    Sys.sleep(time = 1)
}

write.csv(savestuff, "modelruns.csv")
```
...
THis should be done on the overlapping data. We can test it on the whole data as well, but it might not perform well. 
```{r}
...
#overlapping data
newdata2   <- list(xdata = IATTC_dat_overlap_x, nsim = 1000) 
p32 <- gjamPredict(out, newdata = newdata2)
save(p32, file = "Results/p32.rdata")
```
