...
xnames <- c("DEPTH.RANGE",
            "OXY.MESO","salinity.0000m.mean","surf.el.mean","water.temp.0000m.mean","aqua.CHL.chlor.a.mean")

IATTC_dat_nooverlap_x <-  IATTC_dat[,xnames]
IATTC_dat_nooverlap_y <-  IATTC_dat[,cdex]

haw_nooverlap_x <-  haw_x[,xnames]
haw_nooverlap_y <-  haw_y[,cdex]

nooverlap_y <- rbind(IATTC_dat_nooverlap_y, haw_nooverlap_y)
nooverlap_x <- rbind(IATTC_dat_nooverlap_x, haw_nooverlap_x)

formula <- as.formula(~water.temp.0000m.mean +aqua.CHL.chlor.a.mean + surf.el.mean + OXY.MESO + salinity.0000m.mean*aqua.CHL.chlor.a.mean + DEPTH.RANGE)

ml  <- list(ng = 200, burnin = 80, typeNames = 'CA')#, effort = effort)#, effort = effort) # fit the non-holdouts
ml$REDUCT <- FALSE

out<- gjam(formula = formula, nooverlap_x, nooverlap_y, modelList = ml)
