...
```{r}
#test with 1000, 
haw_y_1000 <- haw_y*1000
haw_y <- haw_y_1000
haw_dat <- cbind(haw_y_1000, haw_x)
###1.1.2 Split into Testing and Training Data
...
out<- gjam(formula = formula, haw_train_x, haw_train_y, modelList = ml)
save(out, file = "Models/trained_hawaii.rdata")
load("Models/trained_hawaii.rdata")
```
...
p <- ggplot()  + geom_point(data = dat, aes(x = pred,y = obs), size = .2, color = "grey50", alpha = .3) +
...
pp <- ggplot()  + geom_point(data = dat, aes(x = obs,y = pred), size = .2, color = "grey50", alpha = .3) +
  # geom_point(data = df,aes(x = x,y = y, colour=(ymax-ymin)/2)) + 
  # geom_errorbar(data = df,aes(x = x,y = y, ymin = ymin,ymax = ymax,width=0.1,colour = ((ymax-ymin)/2))) + 
  geom_abline(linetype=2) + 
  scale_color_gradientn(colours = pal) +  
...
ggarrange(p, pp, common.legend = TRUE, legend = "bottom")+ ggsave(filename= "Figures/1000/FigureS2.jpg", plot=last_plot(), width = 25, height=15, units=c("cm"), dpi=500)
```
...
ploty <- ggplot()  + geom_point(data = dat, aes(x = yo,y = yp), size = .2, color = "grey50", alpha = .3) +
  ...
  ylim(c(0,max(dat$yp)))+
  labs(colour = "+/- 1 SD") + 
  annotate(geom="text", color = "red", x=max(dat$yp)/2, y=max(dat$yp), label=paste0("RMSPE =", round(rmseval, digits = 2))) + 
  annotate(geom="text", color = "red", x=max(dat$yp)/2, y=max(dat$yp)-(max(dat$yp)/10), label=paste0("Mean =", round(meany, digits = 2))) + 
  theme(aspect.ratio = 1) +
...
do.call(ggarrange, c(plot_list[1:length(plot_list)], common.legend = TRUE, legend = "bottom"))+ ggsave(filename= "Figures/1000/FigureS4a.jpg", plot=last_plot(), width = 30, height=25, units=c("cm"), dpi=500)
```
...
do.call(ggarrange, c(plot_list[1:length(cdex)], common.legend = TRUE, legend = "bottom"))+ ggsave(filename= "Figures/1000/FigureS_unconditional_outofsample_haw.jpg", plot=last_plot(), width = 30, height=25, units=c("cm"), dpi=500)
```
...
ploty <- ggplot()  + geom_point(data = dat, aes(x = yo,y = yp), size = .2, color = "grey50", alpha = .3)+
  ...
  annotate(geom="text", color = "red", x=max(dat$yp)/10, y=(max(dat$yp)/10), label=paste0("Mean =", round(meany, digits = 2))) + 
  theme(aspect.ratio = 1) +
  labs(y = expression(paste("Predicted catch (CPUE)")), x = expression(paste("Reported catch (CPUE)")),title=name)
#ggsave(ploty, file=paste0("plot_", i,".jpg"), width = 10, height=10, units=c("cm"), dpi=500)
plot_list[[i]] <- ploty
...
do.call(ggarrange, c(plot_list[1:length(plot_list)], common.legend = TRUE, legend = "bottom"))+ ggsave(filename= "Figures/1000/FigureS_unconditional_outofsample_IATTC.jpg", plot=last_plot(), width = 30, height=25, units=c("cm"), dpi=500)
```