...
xnames <- c("Month","Year","DEPTH.RANGE",
...
IATTC_tot$hooks <- IATTC_catch$Hooks_x
IATTC_tot <- IATTC_tot[IATTC_tot$hooks>0,]
IATTC_hooks <- IATTC_catch$Hooks_x
...
p <- ggplot()  + geom_point(data = dat, aes(x = pred,y = obs), size = .2, color = "grey50", alpha = .3) +
  ...
  theme(aspect.ratio = 1) + labs(y = expression(paste("Predicted shark catch (Individuals/1000 hooks)"^"1/2")), x = expression(paste("Reported shark catch (Individuals/1000 hooks)"^"1/2")),title="HPLF Model Validation")
...
pp <- ggplot()  + geom_point(data = dat, aes(x = obs,y = pred), size = .2, color = "grey50", alpha = .3) +
  ...
  labs(x = expression(paste("Predicted shark catch (Individuals/1000 hooks)"^"1/2")), y = expression(paste("Recorded shark catch (Individuals/1000 hooks)"^"1/2")),title="HPLF Model Validation") 
...
ploty <- ggplot()  + geom_point(data = dat, aes(x = yo,y = yp), size = .2, color = "grey50", alpha = .3)+
  ...
  annotate(geom="text", color = "red", x=max(dat$yp)/5, y=max(dat$yp)/8, label=paste0("RMSPE =", round(rmseval, digits = 2))) + 
  annotate(geom="text", color = "red", x=max(dat$yp)/5, y=max(dat$yp)-(max(dat$yp)/10), label=paste0("Mean =", round(meany, digits = 2))) + 
  theme(aspect.ratio = 1) +
  labs(y = expression(paste("Predicted catch (Individuals/1000 hooks)"^"1/2")), x = expression(paste("Reported catch (Individuals/1000 hooks)"^"1/2")),title=name)
#ggsave(ploty, file=paste0("plot_", i,".jpg"), width = 10, height=10, units=c("cm"), dpi=500)
plot_list[[i]] <- ploty
...
do.call(ggarrange, c(plot_list[1:length(plot_list)], common.legend = TRUE, legend = "bottom"))+ ggsave(filename= "Figures/FigureS_unconditional_outofsample_IATTC.jpg", plot=last_plot(), width = 30, height=25, units=c("cm"), dpi=500)
```